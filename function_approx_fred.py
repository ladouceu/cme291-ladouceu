...
# for my linear interpolation class
from scipy.interpolate import interp1d
# from sklearn.isotonic import IsotonicRegression # found a quicker way
...
class BSplineApprox(FunctionApprox[X]):
    ...
    def evaluate(self, x_values_seq: Iterable[X]) -> np.ndarray:      
        spline_func: Callable[[Sequence[float]], np.ndarray] =\
            BSpline(self.knots, self.coeffs, self.degree, extrapolate=True) 
        
        feature_vals: Sequence[float] = self.get_feature_values(x_values_seq)
        pred_vals: np.ndarray = spline_func(feature_vals)

        if self.mode == "":
            return pred_vals ## this mode is broken code, doesn't account for nans
        else: # this conditional statement is for the case were there is monotonic constraint
            sorted_indices = np.argsort(feature_vals)
            sorted_feature_vals = np.array(feature_vals)[sorted_indices]
            unsorted_indices = np.argsort(sorted_indices)
            sorted_pred_vals = pred_vals[sorted_indices]
            mono_pred_vals = [sorted_pred_vals[0]]
            if self.mode == "increasing":
                largest_val = sorted_pred_vals[0]
                for i in range(1,len(sorted_pred_vals)):
                    if sorted_pred_vals[i] > largest_val:
                        largest_val = sorted_pred_vals[i]
                    mono_pred_vals += [largest_val]
                pred_vals =  np.array(mono_pred_vals)[unsorted_indices]

            elif self.mode == "decreasing":
                smallest_val = sorted_pred_vals[0]
                for i in range(1,len(sorted_pred_vals)):
                    if sorted_pred_vals[i] < smallest_val:
                        smallest_val = sorted_pred_vals[i]
                    mono_pred_vals += [smallest_val]
                pred_vals =  np.array(mono_pred_vals)[unsorted_indices]

            else:
                raise RuntimeError("The mode {} is not a valid mode.".format(self.mode))
            return pred_vals
        
            
#             # now correct for the extrapolation NaNs
#             n = 5
#             # lower extrapolation
#             lower_ext_xs = []
#             lower_fit_xs = []
#             lower_fit_ys = []
#             for i in range(len(sorted_feature_vals)):
#                 if sorted_pred_vals[i] == np.nan:
#                     lower_ext_xs += [sorted_feature_vals[i]]
#                 else:
#                     lower_fit_xs += [sorted_feature_vals[i]]
#                     lower_fit_ys += [sorted_pred_vals[i]]
#                 if len(lower_fit_xs) == n:
#                     break
#             if len(lower_ext_xs) > 0:
#                 #OLS
#                 lfx = np.array([lower_fit_xs, [1]*len(lower_fit_xs)]).T
#                 lfy = np.array([lower_fit_ys]).T
#                 lex = np.array([lower_ext_xs]).T
#                 ley = (np.dot(lfx.T, lfy) / np.dot(lfx.T,lfx)) * lex 
#                 # replace the nans with the linear fit predictions
#                 pred_vals = np.stack((pred_vals[len(ley):], ley.flatten()))

#             # upper extrapolation
#             upper_ext_xs = []
#             upper_fit_xs = []
#             upper_fit_ys = []
#             for i in range(0,len(sorted_feature_vals),-1):
#                 if sorted_pred_vals[i] == np.nan:
#                     upper_ext_xs += [sorted_feature_vals[i]]
#                 else:
#                     upper_fit_xs += [sorted_feature_vals[i]]
#                     upper_fit_ys += [sorted_pred_vals[i]]
#                 if len(upper_fit_xs) == n:
#                     break
#             if len(upper_ext_xs) > 0:
#                 # OLS
#                 ufx = np.array([upper_fit_xs, [1]*len(upper_fit_xs)]).T
#                 ufy = np.array([upper_fit_ys]).T
#                 uex = np.array([upper_ext_xs[::-1]]).T # the [::-1] is to unreverse the order
#                 uey = (np.dot(ufx.T, ufy) / np.dot(ufx.T,ufx)) * uex
#                 # replace the nans with the linear fit predictions
#                 pred_vals = np.stack((pred_vals[:-len(uey)], uey.flatten()))
            
#             return pred_vals[unsorted_indices]
    ...
    def __add__(self, other: BSplineApprox[X]) -> \
            BSplineApprox[X]:
        """
    ...
    def __mul__(self, scalar: float) -> BSplineApprox[X]:
...
@dataclass(frozen=True)
class LinearInterpolationApprox(FunctionApprox[X]):

    xs: List[X] = field(default_factory=lambda: [])
    ys: List[float] = field(default_factory=lambda: [])
    feature_function: Callable[[X],float] = field(default_factory=lambda x: x)
    mode:str = field(default_factory="")
    ...
    def create(
               ...
               feature_function: Callable[[X], float],
               mode: str = ""
              ) -> LinearInterpolationApprox[X]:
        x_vals, y_vals = zip(*xy_vals_seq)
        feature_vals: Sequence[float] = [feature_function(x) for x in x_vals]
        return LinearInterpolationApprox(
            feature_vals,
            y_vals,
            feature_function,
            mode=mode)
    ...
    def evaluate(self, x_values_seq: Iterable[X]) -> np.ndarray:
        lin_interp_func: Callable[[Sequence[float]], np.ndarray] = \
            interp1d(self.xs,self.ys, fill_value = "extrapolate")
        feature_vals: Sequence[float] = self.get_feature_values(x_values_seq)
        pred_vals: np.ndarray = lin_interp_func(feature_vals)
        if self.mode == "":
            return pred_vals
        else:
            sorted_indices = np.argsort(feature_vals)
            ...
            if self.mode == "increasing":
                ...
                for i in range(1,len(sorted_pred_vals)):
                    if sorted_pred_vals[i] > largest_val:
                        largest_val = sorted_pred_vals[i]
                    mono_pred_vals += [largest_val]
                return np.array(mono_pred_vals)[unsorted_indices]
    #             return IsotonicRegression(increasing=True).fit_transform(feature_vals, pred_vals)  # the other approach above is quicker
            elif self.mode == "decreasing":
                smallest_val = sorted_pred_vals[0]
                for i in range(1,len(sorted_pred_vals)):
                    if sorted_pred_vals[i] < smallest_val:
                    ...
                    mono_pred_vals += [smallest_val]
                return np.array(mono_pred_vals)[unsorted_indices]
    #             return IsotonicRegression(increasing=False).fit_transform(feature_vals, pred_vals)   # the other approach above is quicker
            else:
                raise RuntimeError("The mode {} is not a valid mode.".format(self.mode))
                return
    ...
    def objective_gradient(
        ...
        xy_vals_seq: Iterable[Tuple[X, float]],
        obj_deriv_out_fun: Callable[[Sequence[X], Sequence[float]], float]
    ) -> Gradient[LinearInterpolationApprox[X]]:
        """
        This is a dummy function.
        """
        return

    def __add__(self, other: LinearInterpolationApprox[X]) -> \
            LinearInterpolationApprox[X]:
        assert(self.feature_function == other.feature_function)
        return LinearInterpolationApprox(self.xs + other.xs,
                                         self.ys + other.ys,
                                         self.feature_function)
        

    def __mul__(self, scalar: float) -> LinearInterpolationApprox[X]:
        return LinearInterpolationApprox(xs,
                                         scalar * ys,
                                         self.feature_function)
    ...
    def solve(
        ...
        return self.create(xy_vals_seq,
                           self.feature_function,
                           self.mode)
